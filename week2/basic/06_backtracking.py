...
def combinations(n, k):
    ...
    def backtrack(start, current_combination):
        ...
        if len(current_combination) == k: # 현재까지 선택한 숫자들의 갯수가 선택할 개수가 되면
            result.append(current_combination[:]) # 리스트는 참조전달 - 슬라이싱으로 리스트 복사
        ...
        for i in range(start, n+1):
            ...
            current_combination.pop() # 재귀가 끝나고 선택 안 한 상태로 되돌아감, 리셋
